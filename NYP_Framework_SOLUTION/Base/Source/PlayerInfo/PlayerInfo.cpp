...
CPlayerInfo::CPlayerInfo(void)
	...
	, m_dCameraHeight(10.0)
	, m_bFallDownwards(false)
	, m_dFallSpeed(0.0)
	, m_dFallAcceleration(-20.0)
	, m_dCameraRotationSpeed(120.0)
	, m_bLanded(true)
	, attachedCamera(NULL)
	, m_pTerrain(NULL)
	, primaryWeapon(NULL)
...
void CPlayerInfo::Init(void)
{
	...
	defaultPosition.Set(0,m_dCameraHeight,10);
	defaultTarget.Set(0,0,0);
	defaultUp.Set(0,1,0);

	// Set the current values
	position.Set(0, m_dCameraHeight, 10);
	target.Set(0, m_dCameraHeight, 0);
	up.Set(0, 1, 0);
...
void CPlayerInfo::UpdateFreeFall(double dt)
{
	...
	if (m_bLanded == false && position.y - m_dCameraHeight < 0)
	{
		Vector3 viewDirection = target - position;
		position.y = m_dCameraHeight;
...
// Update Jump Upwards
void CPlayerInfo::UpdateJumpUpwards(double dt, Vector3& nextPosition)
{
	if (m_bJumpUpwards == false)
		return;
	...
	// Use SUVAT equation to update the change in position and target
	// s = u * t + 0.5 * a * t ^ 2
	nextPosition.y += (float)(m_dJumpSpeed * dt + 0.5 * m_dJumpAcceleration * dt * dt);
	// Use this equation to calculate final velocity, v
	// SUVAT: v = u + a * t;
	// v is m_dJumpSpeed AFTER updating using SUVAT where u is the initial speed and is equal to m_dJumpSpeed
	m_dJumpSpeed = m_dJumpSpeed + m_dJumpAcceleration * dt;
...
// Update FreeFall
void CPlayerInfo::UpdateFreeFall(double dt, Vector3& nextPosition)
{
	if (m_bFallDownwards == false)
		return;
	...
	// Use SUVAT equation to update the change in position and target
	// s = u * t + 0.5 * a * t ^ 2
	nextPosition.y += (float)(m_dFallSpeed * dt + 0.5 * m_dJumpAcceleration * dt * dt);
	// Use this equation to calculate final velocity, v
	// SUVAT: v = u + a * t;
	// v is m_dJumpSpeed AFTER updating using SUVAT where u is the initial speed and is equal to m_dJumpSpeed
	m_dFallSpeed = m_dFallSpeed + m_dFallAcceleration * dt;
...
void CPlayerInfo::CameraUpdate(double dt)
{
	...
	{
		UpdateJumpUpwards(dt, nextPosition);
		UpdateFreeFall(dt, nextPosition);
	}
		...
		KeyboardController::GetInstance()->IsKeyDown('D'))
	{
		...
		if (KeyboardController::GetInstance()->IsKeyDown('W'))
		{
			Vector3 direction(viewVector.x, 0.f, viewVector.z);
			nextPosition += direction.Normalized() * (float)m_dSpeed * (float)dt;
		}
		...
		if (KeyboardController::GetInstance()->IsKeyDown('A'))
		{
			...
			rightUV.y = 0;
			rightUV.Normalize();
			nextPosition -= rightUV * (float)m_dSpeed * (float)dt;
		}
	...
	//Update the camera direction based on mouse move
	{
		...
		{
			float yaw = (float)(-m_dCameraRotationSpeed * camera_yaw * (float)dt);
			Mtx44 rotation;
			rotation.SetToRotation(yaw, 0, 1, 0);
			viewUV = rotation * viewUV;
			target = position + viewUV;
		...
		{
			float pitch = (float)(-m_dCameraRotationSpeed * camera_pitch * (float)dt);
			rightUV = viewUV.Cross(up);
			rightUV.y = 0;
			rightUV.Normalize();
			up = rightUV.Cross(viewUV).Normalized();
...
void CPlayerInfo::CollisionUpdate(Vector3& nextPos)
{
	...
	//cout << "List Size: " << ObjectList.size() << endl;
	...
	for (int i = 0; i < ObjectList.size(); i++)
	{
		if (ObjectList[i]->HasCollider())
		{
			if (moveX)
			{
				if (nextPos.y > ObjectList[i]->GetMin().y - offset && nextPos.y < ObjectList[i]->GetMax().y + offset + m_dCameraHeight)
				{
					if (nextPos.x > ObjectList[i]->GetMin().x - offset && nextPos.x < ObjectList[i]->GetMax().x + offset)
					{
						if ((position.z > ObjectList[i]->GetMin().z - offset && position.z < ObjectList[i]->GetMax().z + offset))
						{
							moveX = false;
						}
					}
			...
			if (moveZ)
			{
				if (nextPos.y > ObjectList[i]->GetMin().y - offset && nextPos.y < ObjectList[i]->GetMax().y + offset + m_dCameraHeight)
				{
					if (nextPos.z > ObjectList[i]->GetMin().z - offset && nextPos.z < ObjectList[i]->GetMax().z + offset)
					{
						if ((position.x > ObjectList[i]->GetMin().x - offset && position.x < ObjectList[i]->GetMax().x + offset))
						{
							moveZ = false;
						}
					}
			...
			if (moveY)
			{
				...
				{
					if ((position.x > ObjectList[i]->GetMin().x - offset && position.x < ObjectList[i]->GetMax().x + offset) &&
						(position.z > ObjectList[i]->GetMin().z - offset && position.z < ObjectList[i]->GetMax().z + offset))
					{
						moveY = false;
					}
				}
	...
	{
		if (m_bLanded && position.y - m_dCameraHeight > 1)
		{
			m_bLanded = false;
			SetOnFreeFall(true);
		}
