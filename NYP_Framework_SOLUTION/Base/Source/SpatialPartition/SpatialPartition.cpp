...
void CSpatialPartition::Add(EntityBase* theObject)
{
	if (!theObject->HasCollider())
	{
		...
		{
			if (!theGrid[xIndex*zNumOfGrid + zIndex].IsHere(theObject))
			theGrid[xIndex*zNumOfGrid + zIndex].Add(theObject);
		}
	}
	else
	{
		Vector3 Min = theObject->GetMin();
		Vector3 Max = theObject->GetMax();
		int Min_xIndex = (((int)Min.x - (-xSize >> 1)) / (xSize / xNumOfGrid));
		int Min_zIndex = (((int)Min.z - (-zSize >> 1)) / (zSize / zNumOfGrid));
		int Max_xIndex = (((int)Max.x - (-xSize >> 1)) / (xSize / xNumOfGrid));
		int Max_zIndex = (((int)Max.z - (-zSize >> 1)) / (zSize / zNumOfGrid));

		for (int x = Min_xIndex; x <= Max_xIndex; x++)
		{
			for (int z = Min_zIndex; z <= Max_zIndex; z++)
			{
				if (((x >= 0) && (x < xNumOfGrid)) && ((z >= 0) && (z < zNumOfGrid)))
				{
					if (!theGrid[x * zNumOfGrid + z].IsHere(theObject))
					theGrid[x * zNumOfGrid + z].Add(theObject);
				}
			}
		}
	}
	...
	else
	{
		...
		for (int x = Min_xIndex; x <= Max_xIndex; x++)
		{
			for (int z = Min_zIndex; z <= Max_zIndex; z++)
			{
				if (((x >= 0) && (x < xNumOfGrid)) && ((z >= 0) && (z < zNumOfGrid)))
				{
					theGrid[x * zNumOfGrid + z].Remove (theObject);
				}
			}
		}
	}
